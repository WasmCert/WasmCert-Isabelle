...
lemma
  ...
  shows hoare_triple_effect: "success c h \<Longrightarrow> \<exists>h' r. effect c h h' r \<and> (h',new_addrs h as h')\<Turnstile>Q r"
  using assms 
...
proof -
  ...
  from hoare_tripleD(1)[OF A M1 RUN] have 
    "(the_state \<sigma>, new_addrs h as1 (the_state \<sigma>)) \<Turnstile> Q r" by simp
...
proof -
  ...
  from hoare_tripleD[OF T1 M R1] have  
        M': "(the_state \<sigma>', new_addrs h as (the_state \<sigma>')) \<Turnstile> R r'"
