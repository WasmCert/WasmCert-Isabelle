...
| memory_fill_done: "\<lbrakk>smem_ind (f_inst f) = Some ma; (mems s)!ma = m; dest \<le> mem_length m \<rbrakk> \<Longrightarrow> \<lparr>s;f;[$EConstNum (ConstInt32 (int_of_nat dest)), $EConstNum (ConstInt32 val), $EConstNum (ConstInt32 (int_of_nat n)),$Memory_fill]\<rparr> \<leadsto> \<lparr>s;f;[]\<rparr>"
