...
| memory_init: "\<lbrakk>smem_ind (f_inst f) = Some ma; (mems s)!ma = m; x < length (inst.datas (f_inst f)); da =(inst.datas (f_inst f))!x; dat = (datas s)!da; ndest = nat_of_int dest; nsrc = nat_of_int src; nn = nat_of_int n; nsrc+nn \<le> length dat;  ndest+nn \<le> mem_length m; nn = 0; nsrc+nn \<le> length dat;  ndest+nn \<le> mem_length m; nn = nn_pred+1; b = nat_of_uint8 (dat!nsrc) \<rbrakk> \<Longrightarrow>
...
| memory_copy_done: "\<lbrakk>smem_ind (f_inst f) = Some ma; (mems s)!ma = m; ndest = nat_of_int dest; nsrc = nat_of_int src; nsrc+nn \<le> mem_length m; ndest+nn \<le> mem_length m; nn=0 \<rbrakk> \<Longrightarrow> \<lparr>s;f;[$EConstNum (ConstInt32 dest), $EConstNum (ConstInt32 src), $EConstNum (ConstInt32 n),$Memory_copy]\<rparr> \<leadsto> \<lparr>s;f;[]\<rparr>"
| memory_copy_1: "\<lbrakk>smem_ind (f_inst f) = Some ma; (mems s)!ma = m; ndest = nat_of_int dest; nsrc = nat_of_int src; nsrc+nn \<le> mem_length m; ndest+nn \<le> mem_length m; nn=nn_pred+1; ndest \<le> nsrc \<rbrakk> \<Longrightarrow> \<lparr>s;f;[$EConstNum (ConstInt32 dest), $EConstNum (ConstInt32 src), $EConstNum (ConstInt32 n), $Memory_copy]\<rparr> \<leadsto> \<lparr>s;f;[$EConstNum (ConstInt32 dest), $EConstNum (ConstInt32 src), $Load T_i32 (Some (Tp_i8, U)) 0 0, $Store T_i32 (Some Tp_i8) 0 0, $EConstNum (ConstInt32 (int_of_nat (ndest+1))), $EConstNum (ConstInt32 (int_of_nat (nsrc+1))), $EConstNum (ConstInt32 n), $Memory_copy]\<rparr>"
| memory_copy_2: "\<lbrakk>smem_ind (f_inst f) = Some ma; (mems s)!ma = m; ndest = nat_of_int dest; nsrc = nat_of_int src; nsrc+nn \<le> mem_length m; ndest+nn \<le> mem_length m; nn=nn_pred+1; ndest > nsrc \<rbrakk> \<Longrightarrow> \<lparr>s;f;[$EConstNum (ConstInt32 dest), $EConstNum (ConstInt32 src), $EConstNum (ConstInt32 n), $Memory_copy]\<rparr> \<leadsto> \<lparr>s;f;[$EConstNum (ConstInt32 (int_of_nat (ndest+nn_pred))), $EConstNum (ConstInt32 (int_of_nat (nsrc+nn_pred))), $Load T_i32 (Some (Tp_i8, U)) 0 0, $Store T_i32 (Some Tp_i8) 0 0, $EConstNum (ConstInt32 dest), $EConstNum (ConstInt32 src), $EConstNum (ConstInt32 (int_of_nat nn_pred)), $Memory_copy]\<rparr>"
