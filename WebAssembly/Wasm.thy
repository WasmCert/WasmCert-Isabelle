...
| table_init_trap: "\<lbrakk>stab_ind (f_inst f) x = Some ta; (tabs s)!ta = tab; x < length (inst.elems (f_inst f)); da = (inst.elems (f_inst f))!x; y < length (inst.elems (f_inst f)); ea = (inst.elems (f_inst f))!y; el = (elems s)!ea; src+n > length (snd el) \<or> dest+n > length (snd tab)\<rbrakk> \<Longrightarrow> \<lparr>s;f;[$EConstNum (ConstInt32 (int_of_nat dest)), $EConstNum (ConstInt32 (int_of_nat src)), $EConstNum (ConstInt32 (int_of_nat n)),$Table_init x y]\<rparr> \<leadsto> \<lparr>s;f;[Trap]\<rparr>"
| table_init_done: "\<lbrakk>stab_ind (f_inst f) x = Some ta; (tabs s)!ta = tab; x < length (inst.elems (f_inst f)); da = (inst.elems (f_inst f))!x; y < length (inst.elems (f_inst f)); ea = (inst.elems (f_inst f))!y; el = (elems s)!ea; src \<le> length (snd el); dest \<le> length (snd tab)\<rbrakk> \<Longrightarrow> \<lparr>s;f;[$EConstNum (ConstInt32 (int_of_nat dest)), $EConstNum (ConstInt32 (int_of_nat src)), $EConstNum (ConstInt32 0),$Table_init x y]\<rparr> \<leadsto> \<lparr>s;f;[]\<rparr>"
| table_init: "\<lbrakk>stab_ind (f_inst f) x = Some ta; (tabs s)!ta = tab; x < length (inst.elems (f_inst f)); da = (inst.elems (f_inst f))!x; y < length (inst.elems (f_inst f)); ea = (inst.elems (f_inst f))!y; el = (elems s)!ea; src+n+1 \<le> length (snd el); dest+n \<le> length (snd tab); val = (snd el)!src\<rbrakk> \<Longrightarrow> \<lparr>s;f;[$EConstNum (ConstInt32 (int_of_nat dest)), $EConstNum (ConstInt32 (int_of_nat src)), $EConstNum (ConstInt32 (int_of_nat (n+1))),$Table_init x y]\<rparr> \<leadsto> \<lparr>s;f;[$EConstNum (ConstInt32 (int_of_nat dest)), $C (V_ref val), $EConstNum (ConstInt32 (int_of_nat (dest+1))), $EConstNum (ConstInt32 (int_of_nat (src+1))), $EConstNum (ConstInt32 (int_of_nat n)), $Table_init x y]\<rparr>"
