  ...
  (vesa, Basic (Br (nth js k)) :: esa)))
                 ...
                 | Basic (Call_indirect j) ->
                   (let i = f_inst f in
                     (match ves with [] -> (s, (f, RSCrash CError))
                       | ConstInt32 c :: vesa ->
                         (match stab s i (nat_of_int_i32 c)
                     ...
                     (match ves with [] -> (s, (f, RSCrash CError))
                       | ConstInt32 k :: vesa ->
                         (match smem_ind s i
                           ...
                           | Some a ->
                             (match
                               load (nth (mems s) a) (nat_of_int_i32 k) off
                                 (t_length t)
                               with None ->
                           ...
                           | Some a ->
                             (match
                               load_packed sx (nth (mems s) a)
                                 (nat_of_int_i32 k) off (tp_length tp)
                                 (t_length t)
                               with None ->
                                 (s, (f, RSNormal (vesa, Trap :: esa)))
                               | Some aa ->
                       ...
                       | v :: ConstInt32 k :: vesa ->
                         (if types_agree t v
                           then (match smem_ind s i
                                  ...
                                  | Some a ->
                                    (match
                                      store (nth (mems s) a) (nat_of_int_i32 k)
off (bits v) (t_length t)
                                      with None ->
(s, (f, RSNormal (vesa, Trap :: esa)))
                                      | Some a_a ->
...
(nat_of_int_i32 k) off (bits v) (tp_length tp)
                                      with None ->
(s, (f, RSNormal (vesa, Trap :: esa)))
                                      | Some a_a ->
(mems_update (fun _ -> list_update (mems s) a a_a) s,
                       ...
                       (match smem_ind s (f_inst f)
                         ...
                         | Some a ->
                           (let l = mem_size (nth (mems s) a) in
                             (match mem_grow (nth (mems s) a) (nat_of_int_i32 c)
                               with None ->
                                 (s, (f, RSNormal
   (ConstInt32 int32_minus_one :: vesa, esa)))
                               | Some a_a ->
                                 (mems_update
                                    (fun _ -> list_update (mems s) a a_a) s,
                                   (f, RSNormal
 (ConstInt32 (int_of_nat_i32 l) :: vesa, esa))))))
                     | ConstInt64 _ :: _ -> (s, (f, RSCrash CError))
                     | ConstFloat32 _ :: _ -> (s, (f, RSCrash CError))
                     | ConstFloat64 _ :: _ -> (s, (f, RSCrash CError)))
